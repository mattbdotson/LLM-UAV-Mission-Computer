...
- **Mission Computer**: NVIDIA Jetson Orin Nano Super Developer Kit, nicknamed "Penny Royal" (Neal Asher Polity universe reference)
  ...
  - 8GB unified memory
  - 64GB microSD (NVMe SSD on order — Crucial P3 500GB)
  - IP: 192.168.1.177
  - Username: matthew
...
- **Pluggable backends** — inference backend is swappable via `.env` (Ollama, VILA, TensorRT)
...
│   │   ├── init.py                # load_backend() factory function
│   │   ├── ollama_backend.py          # Ollama/llama.cpp (text only currently)
...
│   ├── scripts/
│   │   └── start_vila_server.sh       # launches VILA container on Jetson
│   ├── tests/
...
INFERENCE_BACKEND=vila        # ollama | vila | tensorrt
...
- Tile size: 2304×2304px, resized to 384×384 for VLM input (VILA native resolution)
...
## VLM Deployment (Pending NVMe)
- Model: VILA 1.5-3B via dustynv/nano_llm:r36.4.0 Docker container
- Issue: nano_llm container is 12.7GB + VILA weights ~7GB = exceeds 64GB SD card with OS
- Fix: NVMe SSD (Crucial P3 500GB) on order
- VILA server: Flask wrapper in `servers/vila_server.py` exposes `/generate` and `/health`
- Startup: `scripts/start_vila_server.sh` mounts mission_manager folder into container

## Known Issues
- CUDA memory bug in L4T 36.4.7 prevents Ollama from loading models >1B params
- nano_llm/VILA not yet deployed — blocked on NVMe SSD
...
4. ⬜ Deploy VILA on Jetson (blocked on NVMe)
5. ⬜ First VLM vision mission test
6. ⬜ TensorRT backend implementation
7. ⬜ Real autopilot hardware (HIL upgrade)
...
# Start VILA server on Jetson (run on Penny Royal)
bash scripts/start_vila_server.sh

# Switch backend (edit config/.env)
INFERENCE_BACKEND=ollama  # for text-only testing
INFERENCE_BACKEND=vila    # for vision testing
